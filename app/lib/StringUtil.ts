...
export class StringUtil {
   ...
   * @param {String} str 変換したい文字列
   */
  public static br2nl(str: string): string {
   ...
   */
  public static tab2space(str: string, num: number): string {
    //num = parseInt(num, 10);
   ...
   */
  public static snake2camel(str: string, upper: boolean): string {
    str = str
      .replace(/^[\-_ ]/g, "")
      .replace(/[\-_ ]./g, function (match: string) {
        return match.charAt(1).toUpperCase();
      });
    return upper === true ?
      str.replace(/^[a-z]/g, function (match: string) {
   ...
   */
  public static camel2snake(str: string, separator: string): string {
    separator = separator === undefined ? "-" : separator;
    return str
      .replace(/^[A-Z]+$/, function (match: string) {
      ...
      .replace(/[A-Z]+$/g, function (match: string) {
   ...
   */
  public static hira2kana(str: string, opt: boolean): string {
    str = str
      .replace(/[ぁ-ゔ]/g, function (s: string) {
   ...
   */
  public static kana2hira(str: string, opt: boolean): string {
    str = str
      .replace(/[ァ-ヴ]/g, function (s: string) {
        return String.fromCharCode(s.charCodeAt(0) - 0x60);
      })
      .replace(/ﾞ/g, '゛')
      .replace(/ﾟ/g, '゜')
   ...
   * @param {String} str 変換したい文字列
   */
  public static hankana2zenkana(str: string): string {
    var kanaMap = {
   ...
   */
  public static zen2han(str: string, tilde: boolean, mark: boolean, hankana: boolean, space: boolean, alpha: boolean, num: boolean): string {
    if (alpha !== false) {
      str = str.replace(/[Ａ-Ｚａ-ｚ]/g, function (s: string) {
        return String.fromCharCode(s.charCodeAt(0) - 65248);
      });
    }
    if (num !== false) {
      str = str.replace(/[０-９]/g, function (s: string) {
        return String.fromCharCode(s.charCodeAt(0) - 65248);
      });
    }
    if (mark !== false) {
      var reg = /[！＂＃＄％＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾＿｀｛｜｝]/g;
      str = str.replace(reg, function (s: string) {
        return String.fromCharCode(s.charCodeAt(0) - 65248);
      }).replace(/[‐－―]/g, '-');
    }
    if (tilde !== false) {
   ...
   * @param {Boolean} num 数字 falseを指定した場合は変換なし
   */
  public static han2zen(str: string, tilde: boolean, mark: boolean, hankana: boolean, space: boolean, alpha: boolean, num: boolean): string {
    if (alpha !== false) {
      str = str.replace(/[A-Za-z]/g, function (s: string) {
        return String.fromCharCode(s.charCodeAt(0) + 65248);
      });
    }
    if (num !== false) {
      str = str.replace(/\d/g, function (s: string) {
        return String.fromCharCode(s.charCodeAt(0) + 65248);
      });
    }
    if (mark !== false) {
   ...
   */
  public static one2multi(str: string): string {
    var map = {
      '㈱': '(株)', '㈲': '(有)', '㈳': '(社)', '㈵': '(特)',
      '㈶': '(財)', '㈻': '(学)', '㈼': '(監)', '㍿': '株式会社'
    };
   ...
   * @param {String} str 変換したい文字列
   */
  public static removeNl(str: string): string {
    var str: string = str.replace(/\r\n/g, '');
    str = str.replace(/[\r\n]/g, '');
    return str;
  };
   ...
   */
  public static trim(str: string, multipleLines: boolean, useBlankLine: boolean): string {
   ...
   * @param {Boolean} useBlankLine 空行を使用するかどうか（falseの時は空行削除）
   */
  public static rtrim(str: string, multipleLines: boolean, useBlankLine: boolean): string {
    var reg = /[　\s]+$/g;
    if (multipleLines === true) {
      if (useBlankLine === false) {
        str = str
   ...
   * @param {Boolean} useBlankLine 空行を使用するかどうか（falseの時は空行削除）
   */
  public static ltrim(str: string, multipleLines: boolean, useBlankLine: boolean): string {
    var reg = /^[\s　]+/g;
    if (multipleLines === true) {
      if (useBlankLine === false) {
        str = str
        ...
        var split = str.split('\n');
        str = split.map(function (line: string) {
          return line.replace(reg, '');
        }).join('\n').replace(reg, '');
      }
    } else {
